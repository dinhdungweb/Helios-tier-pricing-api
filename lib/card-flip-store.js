...
const DEFAULT_FIRST_TURN_AMOUNT = toNonNegativeNumber(process.env.CARD_FLIP_FIRST_TURN_AMOUNT, 500000);
const DEFAULT_SECOND_TURN_AMOUNT = toNonNegativeNumber(process.env.CARD_FLIP_SECOND_TURN_AMOUNT, 3000000);
...
function getConfig(options = {}) {
  ...
  const firstTurnAmount = DEFAULT_FIRST_TURN_AMOUNT;
  const secondTurnAmount = DEFAULT_SECOND_TURN_AMOUNT;
...
async function getGameState(customerId, campaignId, options = {}) {
  ...
  const [customer, accumulatedOrderAmount, historyMetafield] = await Promise.all([
    getCustomer(numericId),
    getAccumulatedOrderAmount(numericId, config.orderStartDate),
    getCustomerMetafieldSnapshot(numericId, config.historyNamespace, config.historyKey)
  ]);

  const allowedTurns = getAllowedTurns(customer, accumulatedOrderAmount, config);
...
async function getAccumulatedOrderAmount(customerId, startDate) {
  let after = null;
  let total = 0;
  ...
  do {
    ...
    for (const order of orders.nodes || []) {
      ...
      total += toNonNegativeNumber(order.currentTotalPriceSet?.shopMoney?.amount, 0);
    }
...
function calculateAllowedTurns(accumulatedAmount, config) {
  if (accumulatedAmount >= config.secondTurnAmount) return 2;
  if (accumulatedAmount >= config.firstTurnAmount) return 1;
  return 0;
}

function getAllowedTurns(customer, accumulatedAmount, config) {
  ...
  return calculateAllowedTurns(accumulatedAmount, config);
}
...
async function recordPlayOnce(payload) {
  ...
  const entry = {
    ...
    special_card_key: resolvedCard.specialKey || '',
    played_at: playedAt
  };
  ...
  try {
    ...
    if (resolvedCard.specialKey) {
      await releaseSpecialCard(state.campaignId, resolvedCard.specialKey, state.config).catch(() => {});
    }
    throw error;
  }
...
async function resolveAvailableCard(state, selectedCard, deckCards) {
  const specialKey = getSpecialCardKey(selectedCard);
  if (!specialKey) return selectedCard;

  const claimed = await claimSpecialCard(state.campaignId, specialKey, state.config);
  if (claimed) {
    return { ...selectedCard, specialKey };
  }

  const normalCards = deckCards.filter((card) => !getSpecialCardKey(card));
  if (!normalCards.length) {
    throw httpError(409, 'Thẻ đặc biệt đã hết và không có thẻ thường để thay thế.');
  }

  return normalCards[Math.floor(Math.random() * normalCards.length)];
}

async function getSpecialCardInventory(options = {}) {
  assertServerConfig();
...
async function updateSpecialCardInventory(payload = {}) {
  assertServerConfig();
  ...
  await mutateInventory(config, (inventory) => {
    ...
    Object.keys(SPECIAL_CARDS).forEach((key) => {
  ...
  return getSpecialCardInventory({ campaign_id: campaignId });
}

async function claimSpecialCard(campaignId, specialKey, config) {
  const result = await mutateInventory(config, (inventory) => {
    const campaign = normalizeCampaignInventory(inventory[campaignId]);
    const card = campaign[specialKey];

    if (!card || card.claimed >= card.capacity) {
      return { inventory, changed: false, claimed: false };
    }

    card.claimed += 1;
    inventory[campaignId] = campaign;
    return { inventory, claimed: true };
  });

  return !!result.claimed;
}

async function releaseSpecialCard(campaignId, specialKey, config, amount = 1) {
  await mutateInventory(config, (inventory) => {
    const campaign = normalizeCampaignInventory(inventory[campaignId]);
    const card = campaign[specialKey];
...
function serializeCampaignInventory(value) {
  ...
  return Object.keys(SPECIAL_CARDS).map((key) => ({
    key,
    label: SPECIAL_CARDS[key].label,
    capacity: campaign[key].capacity,
    claimed: campaign[key].claimed,
    remaining: Math.max(0, campaign[key].capacity - campaign[key].claimed)
  }));
}
...
function normalizeCampaignInventory(value) {
  ...
  Object.keys(SPECIAL_CARDS).forEach((key) => {
    ...
    const capacity = toNonNegativeInt(card.capacity, 0);
    normalized[key] = {
      capacity,
...
async function deleteGameHistory(payload = {}) {
  ...
  for (const customerId of customerIds) {
    ...
    const nextHistory = history.filter((entry) => {
      ...
      if (shouldDelete && SPECIAL_CARDS[entry.special_card_key]) {
        releasedSpecialCards[entry.special_card_key] = (releasedSpecialCards[entry.special_card_key] || 0) + 1;
      }
      return !shouldDelete;
    });
  ...
  for (const [specialKey, amount] of Object.entries(releasedSpecialCards)) {
    await releaseSpecialCard(campaignId, specialKey, config, amount);
  }
  ...
  return {
    ...
    released_special_cards: releasedSpecialCards
  };
}
...
function getSpecialCardKey(card) {
  const title = cleanString(card?.title, 240);
  return Object.keys(SPECIAL_CARDS).find((key) => SPECIAL_CARDS[key].titlePattern.test(title)) || '';
}
