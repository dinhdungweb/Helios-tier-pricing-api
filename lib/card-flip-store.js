...
const DEFAULT_HISTORY_LIST_LIMIT = toNonNegativeInt(process.env.CARD_FLIP_HISTORY_LIST_LIMIT, 500);
...
async function listGameHistory(options = {}) {
  ...
  const entryLimit = clampInt(options.limit, DEFAULT_HISTORY_LIST_LIMIT, 1, 1000);
  ...
  if (includeCustomerId) {
    try {
      const state = await getGameState(includeCustomerId, campaignId, options);
      state.campaignHistory.forEach((entry) => {
        addHistoryEntry(entryMap, uniqueCustomers, {
          ...entry,
          customer_id: entry.customer_id || state.numericId,
          customer_email: entry.customer_email || state.customer.email || ''
        });
      });
    } catch (error) {
  ...
  return {
    ...
    history: entries.slice(0, entryLimit)
  };
}
